...
def set_initial_probe_marker_properties(probe_marker):
    probe_marker.setPos(get_probe_marker_start_pos_on_img(
...
def get_probe_marker_start_pos_on_img(parent_img_dim, probe_idx) -> Tuple:
    ...
    x, y = (im_center + np.array([min(im_center)*np.cos((probe_idx-1)*2*np.pi/6),
                                 min(im_center)*np.sin((probe_idx-1)*2*np.pi/6)]))  # x,y: 0,0 top left
    return (x, y)
...
for probe_idx, probe_label in enumerate(["A", "B", "C", "D", "E", "F"]):
    ...
    probe_notes_list.append(QtWidgets.QLineEdit(
        placeholderText=f"Notes on probe {probe_label}"))
